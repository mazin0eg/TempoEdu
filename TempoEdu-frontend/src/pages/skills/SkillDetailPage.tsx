...
export default function SkillDetailPage() {
  ...
  const { data: skill, isLoading: loading } = useQuery<Skill | null>({
    queryKey: `skill-${id}`,
    queryFn: async () => {
      if (!id) return null;
  ...
  const { data: reviews = [] } = useQuery<Review[]>({
    queryKey: `reviews-user-${typeof skill?.user === 'object' ? skill.user._id : ''}`,
    queryFn: async () => {
      if (!skill || typeof skill.user !== 'object') return [];
      const { data } = await reviewsApi.getByUser(skill.user._id);
      return data.data;
    },
    enabled: !!skill && typeof skill.user === 'object',
  });
